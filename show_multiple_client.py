...
def show(ref_dir,gen_dir,output_dir):
    ...
    for path in os.listdir(gen_dir):
        ...
        if os.path.isdir(full_path) and re.search('product',full_path):
            #full_path is the directory with all the descriptions of a single product
            c = 0
            for p in os.listdir(full_path):
                full_full_path = os.path.join(full_path, p)
                if os.path.isfile(full_full_path) and re.search('product',full_full_path):
                    with open(full_full_path,'r') as f:
                        gen = f.read()
                    ...
                    if c == 0:
                        ...
                        df = df.append({"url":f"=HYPERLINK(\"{code}\")","tags":feat,"generated-desc":gen,"list-desc":desc},ignore_index=True)
                    else:
                        if (re.search('description\:.+?###',gen,re.DOTALL)):
                            gen = re.search('description\:.+?###',gen,re.DOTALL).group(0)
                        else:
                        ...
                        gen = gen.replace("read more","")
                        gen = gen.replace('[0m','')
                        df = df.append({"url":"-","tags":"-","generated-desc":gen,"list-desc":"-"},ignore_index=True)
                    c+=1   
...
ref_dir = "/Users/niyoush/eval_client/ref/"
gen_dir = "/Users/niyoush/eval_client/gen/"
output_dir = "/Users/niyoush/eval_client/"
